...
contract UniLayerOrderManager is ReentrancyGuard, Ownable{
    ...
    function getOrder(uint256 orderId)external view exists(orderId) returns (
            OrderTypeData orderType,
            address payable maker,
            address tokenIn,
            address tokenOut,
    ...
    function executeOrder(uint256 orderId)external exists(orderId) nonReentrant returns (uint256[] memory) {
        ...
        if (uniLayerFee > 0) {
            uint256 reduceAmount = uniLayerFee.mul(6).div(10);
            staker.deposit{value: uniLayerFee.sub(reduceAmount)}();
        }
    ...
    function placeOrder( OrderTypeData orderType,address tokenIn,address tokenOut,uint256 amountInOffered,uint256 amountOutExpected,uint256 executorFee) external payable nonReentrant returns (uint256) {
    ...
    function proceedOrder(uint256 orderId, OrderStateData nextState)internal returns (bool){
